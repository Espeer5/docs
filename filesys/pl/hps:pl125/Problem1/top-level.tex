...
\noindent \emph{In Maudlin's presentation of the EPR argument, the two-particle
                state
\[\ket{S} = \frac{1}{\sqrt{2}}\ket{z\uparrow}_R\ket{z\downarrow}_L - 
            \frac{1}{\sqrt{2}}\ket{z\downarrow}_R\ket{z\uparrow}_L,\]
                in which the spins are anti-aligned is used to show that:
                \textbf{the results of spin measurements on either particle,
                along any axis, must already be determined before either
                particle has its spin measured.}}
